...
function animate() {
    ...
    if (spawnTimer < 0) {
        ...
        if (gameSpeed >= 30) {
            // Fixing The Ultimate Game Speed.
            gameSpeed = 30;
        }
...
button.addEventListener("click", () => {
    button.blur(); // So that after clicking, the button might not be focused.
...
function touchStart(e) {
    let ev = getPosition(e);
    pointerDown = setInterval(() => {
        if (p.x !== null || p.y !== null) {
            createProjectile(p.x, p.y);
        }
    }, 400);
}
